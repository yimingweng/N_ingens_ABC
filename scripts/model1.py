...
# Usage: python3 ingens_indep_model.py start_seed stop_seed -o out_prefix
...
# the divergence time of the two major lineages are around 1,175,736 years ago, means 587,868 generations ago

for x in range(args.start, args.stop):
    print("woring on seed " + str(x) + ", out of " + str(length))
    ...
    demography.add_population_split(time=touone , derived=["selden"], ancestral="COS")
    demography.add_population_split(time=touone , derived=["army"], ancestral="COS")
    demography.add_population_split(time=touone , derived=["conness"], ancestral="COS")
    ...
    out.write(str(x) + "\t" + str(conness) + "\t" + str(selden) + "\t" + str(army) + "\t" + str(na) + "\t" + str(touone) + "\t" + str(f3) + "\t" + str(s) + "\n")
    out.close()